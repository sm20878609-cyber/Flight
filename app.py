...
if hidden_count > 0:
    st.markdown(
        ...
        f"另有 <b>{hidden_count} 班</b>因分數不足閘値而隱藏。可調低閘値或點擊「展開全部航班」查看。</div>",
        unsafe_allow_html=True
    )
...
with tab1:
    ...
    if total_shown <= 0:
        st.markdown(
            ...
            f"<p>在 AI 分數 ≥ <b>{score_threshold} 分</b> 的條件下，除首選推薦外沒有其他候補航班。可展開全部航班或調低分數閘値。</p></div>",
            unsafe_allow_html=True
        )
    else:
        st.markdown(
